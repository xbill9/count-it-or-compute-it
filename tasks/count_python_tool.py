# %% [markdown]
# # Count It or Compute It: Python tool
#
# Same prompt as the in-context task, with the ids in the prompt, plus a
# `run_python` tool where `ids` is already defined. The model decides whether
# to compute the count or count by hand.
...
# %%
@kbench.task(name="count-python-tool")
def count_python_tool(llm) -> float:
    runs = count_python_tool_row.evaluate(
        llm=[llm], evaluation_data=pd.DataFrame(ROWS), n_jobs=4, on_failure="continue")
    return summarize(runs, len(ROWS), "python-tool")


# %%
@kbench.task(name="count-python-tool-row", store_task=False)
def count_python_tool_row(llm, case_id, size, phrasing, ids, question, truth_where, expected) -> dict:
...
count_python_tool.run(kbench.llm)
