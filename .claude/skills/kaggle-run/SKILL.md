---
name: kaggle-run
description: Push one or more of the count tasks to Kaggle, run them against named models, download the results and report the scores computed from the run files. Use when asked to push, run, rerun or collect results for count-in-context, count-python-tool or count-engine.
---

Arguments: `$ARGUMENTS`: task slugs and model slugs, e.g. `count-engine gemini-2.5-flash claude-haiku-4-5`. With `pending`, download all three tasks and run what `python3 tasks/pending.py results` prints, one task at a time. With no tasks named, use all three. With no models named, ask.
...
3. For each task: `kaggle b t push <slug> -f tasks/<file>.py --wait`
   (`count-in-context` → `count_in_context.py`, `count-python-tool` → `count_python_tool.py`, `count-engine` → `count_engine.py`).
...
7. Run `python3 tasks/summarize.py results`. It reads each `*-row-run_param_id_*.run.json` (per-row dict at `results[0].dictResult`) and prints everything below. Extend the script rather than counting by hand. Report per task and model:
   ...
   - the category counts
   - for `count-engine`, list every `quoted-wrong-filter` and `quoted-no-filter` row with the filter sent and the true filter. Check the filter the model sent, not just the number it quoted.
